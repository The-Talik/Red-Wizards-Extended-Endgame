...
namespace RWEE
{
	public class SimplePopup : MonoBehaviour
	{
		...
		public static void Show(string message, string url = null, int error=0)//1 = warn, 2=error
		{
			if (_inst == null)
			{
			...
			if (error == 2)
				Main.error(_inst._message + (string.IsNullOrEmpty(url) ? "" : " → " + url));
			else if(error == 1)
				Main.warn(_inst._message + (string.IsNullOrEmpty(url) ? "" : " → " + url));
			else
				Main.log(_inst._message + (string.IsNullOrEmpty(url) ? "" : " → " + url));
